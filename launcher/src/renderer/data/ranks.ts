...
export const RANKS: Record<RankId, RankDef> = {
  ...
  crystal_plus: { id: 'crystal_plus', label: 'Nexora+',   colors: ['#5b8af5', '#7c6af5'], icon: 'gem',          priority: 6 },
  member:       { id: 'member',       label: 'Member',     colors: ['#6b7280', '#4b5563'], icon: null,           priority: 9 },
}

// Ranks that unlock Nexora+ perks (premium cosmetics, priority support, etc).
 ...
 * unlocks whatever Nexora+ unlocks.
 */
...
/** Short label for a locked item, e.g. "Nexora+" or "Team". */
export function lockLabel(required: RankId): string {
  return required === 'crystal_plus' ? 'Nexora+' : 'Team'
}
