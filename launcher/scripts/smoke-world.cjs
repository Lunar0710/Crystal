...
;(async () => {
  ...
  const passed = done && pearls && fs.existsSync(shot)
  console.log(`${passed ? 'PASS' : 'FAIL'}: world test finished=${done} keyPearls=${pearls} screenshot=${fs.existsSync(shot) ? shot : 'missing'}`)
